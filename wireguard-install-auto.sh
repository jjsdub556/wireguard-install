...
function installQuestions() {
	...
	echo "IPv4 public address: $SERVER_PUB_IP"
...
#		read -rp "Server WireGuard IPv6: " -e -i fd42:42:42::1 SERVER_WG_IPV6
	...
	until [[ ${SERVER_PORT} =~ ^[0-9]+$ ]] && [ "${SERVER_PORT}" -ge 1 ] && [ "${SERVER_PORT}" -le 65535 ]; do
		...
		echo "Server WireGuard port [1-65535]: $SERVER_PORT"
#		read -rp "Server WireGuard port [1-65535]: " -e -i "${RANDOM_PORT}" SERVER_PORT
	...
	until [[ ${CLIENT_DNS_1} =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
		...
		echo "First DNS resolver to use for the clients: CLIENT_DNS_1"
#		read -rp "First DNS resolver to use for the clients: " -e -i 1.1.1.1 CLIENT_DNS_1
	done
	until [[ ${CLIENT_DNS_2} =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
		CLIENT_DNS_2="8.8.8.8"
		echo "Second DNS resolver to use for the clients: CLIENT_DNS_2"
...
#		echo -e "\nWireGuard uses a parameter called AllowedIPs to determine what is routed over the VPN."
		echo "Allowed IPs list for generated clients (leave default to route everything): $ALLOWED_IPS"
